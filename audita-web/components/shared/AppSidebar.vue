<template>
  ...
  >
    ...
    >
      <div
        class="w-8 h-8 rounded-lg bg-primary flex items-center justify-center text-white font-bold text-sm shadow-sm shrink-0"
      >
        A
      </div>
    ...
    <!-- Nav links -->
    <div
      :class="['flex-1 flex flex-col gap-0.5 overflow-y-auto', sidebarCollapsed ? 'p-2' : 'p-3']"
    >
      <NuxtLink
        to="/dashboard"
        :class="sidebarCollapsed ? 'sidebar-link-rail' : 'sidebar-link'"
        :active-class="sidebarCollapsed ? 'sidebar-link-rail-active' : 'sidebar-link-active'"
        title="Dashboard"
      >
        <svg
    ...
    <div
      :class="['border-t border-outline-variant/50 dark:border-border-dark shrink-0', sidebarCollapsed ? 'p-2' : 'p-3']"
    >
      <!-- New Change — full button or icon-only -->
      <NuxtLink
      ...
      <button
        ...
        @click="toggle"
      >
        <!-- Double-chevron pointing left (collapse) or right (expand when rotated) -->
        <svg
...
// Shared state — also read by default.vue's spacer via CSS variable on <html>
