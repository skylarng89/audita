...
|---|---|---|---|---|---|
| INIT-001 | Initialise Gradle multi-module Spring Boot 4 project (Java 25) | High | 🔴 Not Started | Developer 1 | Modules: `api`, `domain`, `application`, `infrastructure` |
| INIT-002 | Configure HikariCP + Hibernate 7 + PostgreSQL 16 connection | High | 🔴 Not Started | Developer 1 | Use `spring.datasource` + `spring.jpa` properties |
| INIT-003 | Configure Flyway for `public` schema baseline migration | High | 🔴 Not Started | Developer 1 | V1__create_public_schema.sql: tenants, super_admins, tenant_allowed_domains, tenant_sso_configs |
| INIT-004 | Implement `TenantContext` thread-local + Hibernate `CurrentTenantIdentifierResolver` | High | 🔴 Not Started | Developer 1 | Resolves schema from `X-Tenant-Slug` header or subdomain |
| INIT-005 | Implement per-tenant schema Flyway migration runner | High | 🔴 Not Started | Developer 1 | V1__create_tenant_schema.sql with all tenant tables per SRS §3.2 |
| INIT-006 | Configure Spring Boot Actuator health endpoints | Medium | 🔴 Not Started | Developer 1 | `/actuator/health` for Docker/K8s probes |
| INIT-007 | Configure Logback structured JSON logging with MDC (`tenant_id`, `user_id`, `request_id`) | Medium | 🔴 Not Started | Developer 1 | SRS §7.5 |
| INIT-008 | Configure CORS (allow frontend origin only) | High | 🔴 Not Started | Developer 1 | SEC-03 |
| INIT-009 | Set up RFC 7807 Problem Detail global exception handler | High | 🔴 Not Started | Developer 1 | `@ControllerAdvice` |
| INIT-010 | Write Docker Compose: `api`, `web`, `db` services | High | 🔴 Not Started | Developer 1 | Per SRS §10.1 |

### Frontend Scaffold (`audita-web`)

| Task ID | Task | Priority | Status | Assigned To | Notes |
|---|---|---|---|---|---|
| INIT-011 | Initialise Nuxt 3 project with pnpm, TypeScript, Tailwind CSS | High | 🔴 Not Started | Developer 2 | Enable dark mode via `class` strategy |
| INIT-012 | Configure Tailwind design tokens (colours, typography) per design.md | High | 🔴 Not Started | Developer 2 | Navy primary, surface, danger, warning, success palette |
| INIT-013 | Create `default.vue`, `auth.vue`, `platform.vue` layouts | High | 🔴 Not Started | Developer 2 | Sidebar + header for default; split-panel for auth |
| INIT-014 | Create `plugins/api.ts` — `$fetch` wrapper with auth header injection and 401 handling | High | 🔴 Not Started | Developer 2 | Reads access token from `useAuthStore` |
| INIT-015 | Create shared component library baseline: Button, Input, Badge, Card, Modal, Toast, Table, Pagination | High | 🔴 Not Started | Developer 2 | Match ui-designs/ component library screens |
| INIT-016 | Create `middleware/auth.ts` — redirect unauthenticated to login | High | 🔴 Not Started | Developer 2 | —  |
| INIT-017 | Create `middleware/role.ts` — role-based route guard | High | 🔴 Not Started | Developer 2 | Uses `to.meta.requiredRole` |
| INIT-018 | Create `middleware/tenant.ts` — resolve tenant slug from subdomain | Medium | 🔴 Not Started | Developer 2 | Sets `X-Tenant-Slug` header on all API requests |
| INIT-019 | Configure runtime config for API base URL | Medium | 🔴 Not Started | Developer 2 | `NUXT_PUBLIC_API_BASE` env var |
